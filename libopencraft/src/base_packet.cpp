...
opencraft_packet* opencraft_packet::unpack_packet(int proto_state, std::vector<unsigned char> packdata) {
...
std::vector<unsigned char> opencraft_packet::unpack_bytes() {
     int32_t b_len;
     b_len = this->packed.size()-this->bufpos;
     std::vector<unsigned char> retval;
     for(int i=0; i<b_len; i++) {
         retval.push_back(this->packed[i+this->bufpos]);
     }
     return retval;
}
